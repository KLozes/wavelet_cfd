...
int main(int argc, char* argv[]) {
  ...
  i32 nLvls    = argI("--nlvls", (testCase == 1 ? 4 : (testCase == 5 ? 3 : (sodAmr ? 3 : (acoustic ? 3 : (fptbl ? 3 : (afoil ? 6 : 1)))))));
  i32 nBlocksX = argI("--nblocks", (testCase == 1 ? 16 : (testCase == 2 ? 40 : (testCase == 3 ? 8 : (testCase == 4 ? 40 : (testCase == 5 ? 10 : (sodAmr ? 16 : (acoustic ? 64 : (acConv ? 8 : (shear ? 8 : ((ransBox||ransShr||ransWal) ? 4 : (afoil ? 24 : ((fptbl||ibPlate) ? 64 : 100)))))))))))));
  ...
  real domainLenX = (testCase == 2) ? 10.0
                  : (afoil ? (domLenXA > 0 ? domLenXA : 24.0)
                  : ((fptbl||ibPlate) ? (domLenXA > 0 ? domLenXA : 1.5) : 1.0));

  bool cube   = (testCase == 3);
  bool square = (testCase == 1 || testCase == 2 || gresho || sodAmr || acoustic || acConv || shear || ransBox || ransShr || ransWal || afoil);
  ...
  real tEnd  = (testCase == 1 && sodPin > 2.0) ? 0.06*sqrt(10.0/sodPin) : ((testCase == 2) ? 1.0 : (testCase == 3 ? 0.15 : (gresho ? 1.0 : (sodAmr ? 0.15 : (acoustic ? 0.35 : (acConv ? 2.0*acPeriod : (shear ? 0.5 : ((ransBox||ransShr||ransWal) ? 1.0 : (afoil ? 40.0 : ((fptbl||ibPlate) ? 5.0 : 0.20))))))))));
  if (tEndArg > 0) tEnd = tEndArg;                   // CLI override (arg 10)
  real tStep = (testCase == 1) ? ((tEndArg > 0) ? tEnd/50.0 : ((sodPin > 2.0) ? 0.06*sqrt(10.0/sodPin)/10.0 : 0.008)) : (testCase == 2 ? 0.1 : (testCase == 3 ? 0.03 : (gresho ? 0.1 : (sodAmr ? 0.02 : (acoustic ? 0.02 : (acConv ? tEnd : (shear ? tEnd : ((ransBox||ransShr||ransWal) ? tEnd : ((fptbl||ibPlate||afoil) ? tEnd/20.0 : 0.01)))))))));
  ...
  solver->icType          = (testCase == 1 || sodAmr) ? (argI("--dgblast", 0) ? 7 : 1) : (testCase == 2 ? 2 : (testCase == 3 ? 3 : (gresho ? 4 : (acoustic ? 5 : (acConv ? 6 : (shear ? 8 : (ransBox ? 9 : (ransShr ? 10 : (ransWal ? 11 : ((fptbl||ibPlate||afoil) ? 12 : 0))))))))));
  ...
  solver->immerserdBcType = ibPlate ? argI("--ibtype", 2) : 0;
  ...
  solver->ibGhostFree     = argI("--ibgf", ransA ? 1 : 0);
  ...
  // Airfoil body (type 6).  --airfoil takes precedence over --naca.
  {
    ...
    if (nAf > 2) {
      ...
      const real ax0 = argF("--afx", afoil ? 0.5*domainLenX - 0.5*chord + 0.5*hFine
                                           : (real)0.5);
      const real ay0 = argF("--afy", afoil ? 0.5*domainSize[1] + 0.5*hFine : (real)0.5);
  ...
  if (ransA && muArg <= 0 && reArg <= 0)
    printf("[warn] --rans needs a viscous solve; pass --mu or --re\n");
  if (ransA && solver->wallGeom && nLvls > 1)
