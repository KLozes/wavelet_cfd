...
__global__ void initGridKernel(MultiLevelSparseGrid &grid) {
  ...
  bool inDomain = (i < grid.baseGridSize[0]/blockSize + 1 &&
                   ...
                   k < grid.baseGridSize[2]/blockSizeZ + 1);
  ...
  for (i32 di=-1; di<=1 && !near; di++) {
    ...
    if (ni < 0 || nj < 0 || nk < 0 ||
        ...
        nk >= grid.baseGridSize[2]/blockSizeZ) continue;
...
__global__ void addBoundaryBlocksKernel(MultiLevelSparseGrid &grid) {
  ...
  START_BLOCK_LOOP
    ...
    if (!grid.pseudo2D) {
      onBnd = onBnd || (kb == 0 || kb == grid.baseGridSize[2]/blockSizeZ*powi(2,lvl)-1);
    }
...
__global__ void computeImageDataKernel(MultiLevelSparseGrid &grid, i32 f) {
  ...
  START_CELL_LOOP
    ...
    bool onMidPlane;
    if (grid.pseudo2D) {
      onMidPlane = (k == 0);   // pseudo2D renders the LIVE plane: the cell
                               // loop is guarded to k==0, so a mid-plane
                               // index would match no thread at all
    }
    else {
      i32 nPixelsZ = powi(2,(grid.nLvls - 1 - lvl));
      i32 zMid = grid.baseGridSize[2]*powi(2, grid.nLvls-1) / 2;
      i32 kLo = (kb*blockSizeZ + k) * nPixelsZ;
...
__global__ void paintRankGridKernel(MultiLevelSparseGrid &grid, i32 level) {
  START_CELL_LOOP
    ...
    bool onMidPlane;
    if (grid.pseudo2D) onMidPlane = (k == 0);   // pseudo2D renders the LIVE plane: the cell
                               // loop is guarded to k==0, so a mid-plane
                               // index would match no thread at all
    else {
      i32 nPixelsZ = powi(2,(grid.nLvls - 1 - lvl));
...
// pseudo2D: copy the live k == 0 plane into k = 1..blockSize-1.  Deliberately
// does NOT use START_CELL_LOOP -- that macro is exactly what leaves those layers
// stale.  One thread per k=0 cell; each writes blockSize-1 copies per field.
__global__ void broadcastZKernel(MultiLevelSparseGrid &grid, i32 fOnly) {
  const i32 plane = blockSize*blockSize;
  const i32 nPlane = grid.hashTable.nKeys * plane;
  const i32 stride = gridDim.x * blockDim.x;
  for (i32 idx = blockIdx.x*blockDim.x + threadIdx.x; idx < nPlane; idx += stride) {
    const i32 src = (idx / plane)*blockSizeTot + (idx % plane);
    const i32 f0 = (fOnly < 0) ? 0 : fOnly;
    const i32 f1 = (fOnly < 0) ? grid.nFields : fOnly+1;
    for (i32 f = f0; f < f1; f++) {
      real *F = grid.getField(f);
      const real v = F[src];
      for (i32 kk = 1; kk < blockSizeZ; kk++) F[src + kk*plane] = v;
    }
  }
}
